...
Each prestige level (except combat) also adds a +1 multiplier to the bonus received by capes including guild capes (so 2× mult at prestige 1, 3× mult at prestige 2, and so on).
...
> You may have additional time reductions from other sources such as the Chronos Spire building 

### Combat prestige

Prestiging combat skills adds a flat bonus to their effective stat in every session in addition to the XP bonus:

{combat_prestige_table}